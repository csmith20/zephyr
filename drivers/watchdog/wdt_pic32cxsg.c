...
#define DT_DRV_COMPAT microchip_pic32cxsg_watchdog
...
LOG_MODULE_REGISTER(wdt_pic32cxsg);
...
struct wdt_pic32cxsg_dev_data {
	wdt_callback_t cb;
	bool timeout_valid;
};

static struct wdt_pic32cxsg_dev_data wdt_pic32cxsg_data = { 0 };

static void wdt_pic32cxsg_wait_synchronization(void)
{
	while (WDT_SYNCBUSY) {
		/* wait for SYNCBUSY */
	}
...
static inline void wdt_pic32cxsg_set_enable(bool on)
{
...
static inline bool wdt_pic32cxsg_is_enabled(void)
{
#ifdef WDT_CTRLA_ENABLE
...
static uint32_t wdt_pic32cxsg_timeout_to_wdt_period(uint32_t timeout_ms)
{
	uint32_t next_pow2;
	uint32_t cycles;
...
static void wdt_pic32cxsg_isr(const struct device *dev)
{
	struct wdt_pic32cxsg_dev_data *data = dev->data;
...
static int wdt_pic32cxsg_setup(const struct device *dev, uint8_t options)
{
	struct wdt_pic32cxsg_dev_data *data = dev->data;

	if (wdt_pic32cxsg_is_enabled()) {
	...
	/* Enable watchdog */
	wdt_pic32cxsg_set_enable(1);
	wdt_pic32cxsg_wait_synchronization();

	return 0;
}

static int wdt_pic32cxsg_disable(const struct device *dev)
{
	if (!wdt_pic32cxsg_is_enabled()) {
		return -EFAULT;
	}

	wdt_pic32cxsg_set_enable(0);
	wdt_pic32cxsg_wait_synchronization();

	return 0;
}

static int wdt_pic32cxsg_install_timeout(const struct device *dev,
				    const struct wdt_timeout_cfg *cfg)
{
	...
	per = wdt_pic32cxsg_timeout_to_wdt_period(cfg->window.max);
	...
	if (cfg->window.min) {
		/* Window mode */
		window = wdt_pic32cxsg_timeout_to_wdt_period(cfg->window.min);
...
static int wdt_pic32cxsg_feed(const struct device *dev, int channel_id)
{
	struct wdt_pic32cxsg_dev_data *data = dev->data;
...
static const struct wdt_driver_api wdt_pic32cxsg_api = {
	.setup = wdt_pic32cxsg_setup,
	.disable = wdt_pic32cxsg_disable,
	.install_timeout = wdt_pic32cxsg_install_timeout,
	.feed = wdt_pic32cxsg_feed,
};

static int wdt_pic32cxsg_init(const struct device *dev)
{
#ifdef CONFIG_WDT_DISABLE_AT_BOOT
	/* Ignore any errors */
	wdt_pic32cxsg_disable(dev);
#endif
	/* Enable APB clock */
#ifdef MCLK
	...
	IRQ_CONNECT(DT_INST_IRQN(0),
		    DT_INST_IRQ(0, priority), wdt_pic32cxsg_isr,
...
static struct wdt_pic32cxsg_dev_data wdt_pic32cxsg_data;

DEVICE_DT_INST_DEFINE(0, wdt_pic32cxsg_init, NULL,
		    &wdt_pic32cxsg_data, NULL, PRE_KERNEL_1,
		    CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &wdt_pic32cxsg_api);
