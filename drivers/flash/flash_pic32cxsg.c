...
#define DT_DRV_COMPAT microchip_pic32cxsg_nvmctrl
#define SOC_NV_FLASH_NODE DT_INST(0, soc_nv_flash)

#define LOG_LEVEL CONFIG_FLASH_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(flash_pic32cxsg);
...
/*
 * Zephyr and the PIC32CXSG series use different and conflicting names for
 ...
 *
 * This file uses the PIC32CXSG names internally and the Zephyr names in
 * any error messages.
 */
...
struct flash_pic32cxsg_data {
#if CONFIG_SOC_FLASH_PIC32CXSG_EMULATE_BYTE_PAGES
...
static const struct flash_pages_layout flash_pic32cxsg_pages_layout = {
	.pages_count = CONFIG_FLASH_SIZE * 1024 / ROW_SIZE,
	.pages_size = ROW_SIZE,
};
#endif

static const struct flash_parameters flash_pic32cxsg_parameters = {
#if CONFIG_SOC_FLASH_PIC32CXSG_EMULATE_BYTE_PAGES
	.write_block_size = 1,
#else
...
static int flash_pic32cxsg_write_protection(const struct device *dev, bool enable);

static inline void flash_pic32cxsg_sem_take(const struct device *dev)
{
#if defined(CONFIG_MULTITHREADING)
	struct flash_pic32cxsg_data *ctx = dev->data;

	k_sem_take(&ctx->sem, K_FOREVER);
#endif
}

static inline void flash_pic32cxsg_sem_give(const struct device *dev)
{
#if defined(CONFIG_MULTITHREADING)
	struct flash_pic32cxsg_data *ctx = dev->data;
...
static int flash_pic32cxsg_valid_range(off_t offset, size_t len)
{
	if (offset < 0) {
...
static void flash_pic32cxsg_wait_ready(void)
{
...
static int flash_pic32cxsg_check_status(off_t offset)
{
	flash_pic32cxsg_wait_ready();
 ...
 */
static int flash_pic32cxsg_write_page(const struct device *dev, off_t offset,
				 const void *data, size_t len)
{
	...
	err = flash_pic32cxsg_check_status(offset);
	if (err != 0) {
		return err;
	}
...
static int flash_pic32cxsg_erase_row(const struct device *dev, off_t offset)
{
	*FLASH_MEM(offset) = 0U;
...
#endif
	return flash_pic32cxsg_check_status(offset);
}

#if CONFIG_SOC_FLASH_PIC32CXSG_EMULATE_BYTE_PAGES

static int flash_pic32cxsg_commit(const struct device *dev, off_t base)
{
	struct flash_pic32cxsg_data *ctx = dev->data;
	int err;
	int page;

	err = flash_pic32cxsg_erase_row(dev, base);
	if (err != 0) {
		return err;
	}

	for (page = 0; page < PAGES_PER_ROW; page++) {
		err = flash_pic32cxsg_write_page(
...
static int flash_pic32cxsg_write(const struct device *dev, off_t offset,
			    const void *data, size_t len)
{
	struct flash_pic32cxsg_data *ctx = dev->data;
	...
	err = flash_pic32cxsg_valid_range(offset, len);
	if (err != 0) {
		return err;
	}
	...
	flash_pic32cxsg_sem_take(dev);

	err = flash_pic32cxsg_write_protection(dev, false);
	...
	while ((err == 0) && (pos < len)) {
		...
		err = flash_pic32cxsg_commit(dev, base);

		offset += len_step;
		pos    += len_copy;
	}

	int err2 = flash_pic32cxsg_write_protection(dev, true);

	if (!err) {
		err = err2;
	}

	flash_pic32cxsg_sem_give(dev);

	return err;
}

#else /* CONFIG_SOC_FLASH_PIC32CXSG_EMULATE_BYTE_PAGES */
			    ...
			    const void *data, size_t len)
{
	...
	if (err == 0) {
		...
		while (len > 0) {
			err = flash_pic32cxsg_write_page(dev, offset, pdata, write_len);
			if (err != 0) {
				break;
			}
...
static int flash_pic32cxsg_read(const struct device *dev, off_t offset, void *data,
			   size_t len)
{
	int err;
...
static int flash_pic32cxsg_erase(const struct device *dev, off_t offset,
			    size_t size)
{
	int err;

	err = flash_pic32cxsg_valid_range(offset, ROW_SIZE);
	if (err != 0) {
		return err;
	}
	...
	if (err == 0) {
		for (size_t addr = offset; addr < offset + size;
		     addr += ROW_SIZE) {
			err = flash_pic32cxsg_erase_row(dev, addr);
			if (err != 0) {
				break;
			}
		}
...
#if CONFIG_FLASH_PAGE_LAYOUT
void flash_pic32cxsg_page_layout(const struct device *dev,
			    const struct flash_pages_layout **layout,
			    size_t *layout_size)
{
	*layout = &flash_pic32cxsg_pages_layout;
	*layout_size = 1;
}
#endif

static const struct flash_parameters *
flash_pic32cxsg_get_parameters(const struct device *dev)
{
	ARG_UNUSED(dev);

	return &flash_pic32cxsg_parameters;
}

static int flash_pic32cxsg_init(const struct device *dev)
{
#if defined(CONFIG_MULTITHREADING)
	struct flash_pic32cxsg_data *ctx = dev->data;
	...
	return flash_pic32cxsg_write_protection(dev, false);
}

static const struct flash_driver_api flash_pic32cxsg_api = {
	.erase = flash_pic32cxsg_erase,
	.write = flash_pic32cxsg_write,
	.read = flash_pic32cxsg_read,
	.get_parameters = flash_pic32cxsg_get_parameters,
#ifdef CONFIG_FLASH_PAGE_LAYOUT
	.page_layout = flash_pic32cxsg_page_layout,
#endif
};

static struct flash_pic32cxsg_data flash_pic32cxsg_data_0;

DEVICE_DT_INST_DEFINE(0, flash_pic32cxsg_init, NULL,
		    &flash_pic32cxsg_data_0, NULL, POST_KERNEL,
		    CONFIG_FLASH_INIT_PRIORITY, &flash_pic32cxsg_api);
