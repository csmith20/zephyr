...
/** @file
 * @brief Microchip PIC32CXSG MCU family Ethernet MAC (GMAC) driver.
 *
...
#define DT_DRV_COMPAT microchip_pic32cxsg_gmac
#else
#define DT_DRV_COMPAT microchip_pic32cxsg_gmac
#endif

#define LOG_MODULE_NAME eth_pic32cxsg
...
//#include <zephyr/drivers/clock_control/microchip_pic32cxsg_pmc.h>
#include <soc.h>
#include "eth_pic32cxsg_gmac_priv.h"

#include "eth.h"

#ifdef CONFIG_SOC_FAMILY_MICROCHIP_PIC32CXSG
#include "eth_pic32cxsg_gmac.h"
#endif
...
#ifdef CONFIG_SOC_FAMILY_MICROCHIP_PIC32CXSG
#define MCK_FREQ_HZ	SOC_MICROCHIP_PIC32CXSG_MCK_FREQ_HZ
#else
...
#if CONFIG_NET_BUF_DATA_SIZE * CONFIG_ETH_PIC32CXSG_GMAC_BUF_RX_COUNT \
	< GMAC_FRAME_SIZE_MAX
#error CONFIG_NET_BUF_DATA_SIZE * CONFIG_ETH_PIC32CXSG_GMAC_BUF_RX_COUNT is \
	not large enough to hold a full frame
#endif

#if CONFIG_NET_BUF_DATA_SIZE * (CONFIG_NET_BUF_RX_COUNT - \
	CONFIG_ETH_PIC32CXSG_GMAC_BUF_RX_COUNT) < GMAC_FRAME_SIZE_MAX
#error (CONFIG_NET_BUF_RX_COUNT - CONFIG_ETH_PIC32CXSG_GMAC_BUF_RX_COUNT) * \
...
#if (CONFIG_ETH_PIC32CXSG_GMAC_BUF_RX_COUNT + 1) * GMAC_ACTIVE_QUEUE_NUM \
...
#if defined(CONFIG_PTP_CLOCK_PIC32CXSG_GMAC)
...
#if GMAC_ACTIVE_PRIORITY_QUEUE_NUM >= 1
static int eth_pic32cxsg_gmac_setup_qav(Gmac *gmac, int queue_id, bool enable);

static inline void eth_pic32cxsg_gmac_init_qav(Gmac *gmac)
{
	uint32_t idx;

	for (idx = GMAC_QUE_1; idx <= GMAC_ACTIVE_PRIORITY_QUEUE_NUM; idx++) {
		eth_pic32cxsg_gmac_setup_qav(gmac, idx, true);
	}
}

#else

#define eth_pic32cxsg_gmac_init_qav(gmac)
...
static inline struct net_if *get_iface(struct eth_pic32cxsg_dev_data *ctx)
{
	return ctx->iface;
}
...
static void tx_completed(Gmac *gmac, struct gmac_queue *queue)
{
	...
	struct net_pkt *pkt;
	struct gptp_hdr *hdr;
	struct eth_pic32cxsg_dev_data *dev_data =
		CONTAINER_OF(queue, struct eth_pic32cxsg_dev_data,
...
static int eth_pic32cxsg_gmac_get_qav_status(Gmac *gmac, int queue_id, bool *enabled)
{
	/* Verify queue id */
	if (queue_id < GMAC_QUE_1 || queue_id > GMAC_ACTIVE_PRIORITY_QUEUE_NUM) {
		return -EINVAL;
...
static int eth_pic32cxsg_gmac_setup_qav_idle_slope(Gmac *gmac, int queue_id,
...
static uint32_t eth_pic32cxsg_gmac_get_bandwidth(Gmac *gmac)
{
	uint32_t bandwidth;
...
static int eth_pic32cxsg_gmac_get_qav_idle_slope(Gmac *gmac, int queue_id,
					   unsigned int *idle_slope)
{
	/* Verify queue id */
	if (queue_id < GMAC_QUE_1 || queue_id > GMAC_ACTIVE_PRIORITY_QUEUE_NUM) {
...
static int eth_pic32cxsg_gmac_get_qav_delta_bandwidth(Gmac *gmac, int queue_id,
						unsigned int *delta_bandwidth)
{
	...
	ret = eth_pic32cxsg_gmac_get_qav_idle_slope(gmac, queue_id, &idle_slope);
	if (ret) {
		return ret;
	}
	...
	bandwidth = eth_pic32cxsg_gmac_get_bandwidth(gmac);
...
static int eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(Gmac *gmac, int queue_id,
						  int queue_share)
{
	uint32_t bandwidth;
	uint32_t idle_slope;
	...
	return eth_pic32cxsg_gmac_setup_qav_idle_slope(gmac, queue_id, idle_slope);
}
#endif
...
static int gmac_init(Gmac *gmac, uint32_t gmac_ncfgr_val)
{
	...
	/* Default (RMII) is defined at microchip,gmac-common.yaml file */
	 ...
	 */
#if GMAC_ACTIVE_PRIORITY_QUEUE_NUM == 1
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 1, 75);
	 ...
	 * This does not work like that in PIC32CXSG GMAC - the lower priority queues
	 ...
	 */
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 1, 25);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 2, 50);
#elif GMAC_ACTIVE_PRIORITY_QUEUE_NUM == 3
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 1, 25);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 2, 25);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 3, 25);
#elif GMAC_ACTIVE_PRIORITY_QUEUE_NUM == 4
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 1, 21);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 2, 18);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 3, 18);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 4, 18);
#elif GMAC_ACTIVE_PRIORITY_QUEUE_NUM == 5
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 1, 15);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 2, 15);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 3, 15);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 4, 15);
	eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, 5, 15);
#endif

	eth_pic32cxsg_gmac_init_qav(gmac);
...
static void eth_rx(struct gmac_queue *queue)
{
	...
	const struct eth_pic32cxsg_dev_cfg *const cfg = dev->config;
	Gmac *gmac = cfg->regs;
	struct gptp_hdr *hdr;
#endif
...
#if !defined(CONFIG_ETH_PIC32CXSG_GMAC_FORCE_QUEUE) && \
...
{
	const struct eth_pic32cxsg_dev_cfg *const cfg = dev->config;
	struct eth_pic32cxsg_dev_data *const dev_data = dev->data;
	Gmac *gmac = cfg->regs;
	struct gmac_queue *queue;
	...
	queue = &dev_data->queue_list[CONFIG_ETH_PIC32CXSG_GMAC_FORCED_QUEUE];
...
static int eth_initialize(const struct device *dev)
{
	...
	(void)clock_control_on(PIC32CXSG_DT_PMC_CONTROLLER,
...
{
	uint32_t iaddr = CONFIG_ETH_PIC32CXSG_GMAC_MAC_I2C_INT_ADDRESS;
	int ret;
	...
	ret = i2c_write_read_dt(&i2c,
			   &iaddr, CONFIG_ETH_PIC32CXSG_GMAC_MAC_I2C_INT_ADDRESS_SIZE,
			   mac_addr, 6);
...
static void generate_mac(uint8_t mac_addr[6])
{
	...
	gen_random_mac(mac_addr, MICROCHIP_OUI_B0, MICROCHIP_OUI_B1, MICROCHIP_OUI_B2);
#endif
}
...
static void eth0_iface_init(struct net_if *iface)
{
	...
	for (i = 0; i < CONFIG_NET_TC_RX_COUNT; ++i) {
		cfg->regs->GMAC_ST1RPQ[i] =
			GMAC_ST1RPQ_DSTCM(i) |
			GMAC_ST1RPQ_QNB(CONFIG_ETH_PIC32CXSG_GMAC_FORCED_QUEUE);
	}
...
static enum ethernet_hw_caps eth_pic32cxsg_gmac_get_capabilities(const struct device *dev)
{
	ARG_UNUSED(dev);
...
#if GMAC_ACTIVE_PRIORITY_QUEUE_NUM >= 1
static int eth_pic32cxsg_gmac_set_qav_param(const struct device *dev,
				      ...
				      const struct ethernet_config *config)
{
	...
	/* Priority queue IDs start from 1 for PIC32CXSG GMAC */
	...
	case ETHERNET_QAV_PARAM_TYPE_STATUS:
		...
		return eth_pic32cxsg_gmac_setup_qav(gmac, queue_id, enable);
	case ETHERNET_QAV_PARAM_TYPE_DELTA_BANDWIDTH:
		delta_bandwidth = config->qav_param.delta_bandwidth;

		return eth_pic32cxsg_gmac_setup_qav_delta_bandwidth(gmac, queue_id,
							      delta_bandwidth);
	case ETHERNET_QAV_PARAM_TYPE_IDLE_SLOPE:
		idle_slope = config->qav_param.idle_slope;

		/* The standard uses bps, PIC32CXSG GMAC uses Bps - convert now */
		idle_slope /= 8U;

		return eth_pic32cxsg_gmac_setup_qav_idle_slope(gmac, queue_id,
							 idle_slope);
	default:
		break;
	}
...
static int eth_pic32cxsg_gmac_set_config(const struct device *dev,
				   enum ethernet_config_type type,
				   const struct ethernet_config *config)
{
	int result = 0;
	...
	case ETHERNET_CONFIG_TYPE_QAV_PARAM:
		return eth_pic32cxsg_gmac_set_qav_param(dev, type, config);
#endif
...
#if GMAC_ACTIVE_PRIORITY_QUEUE_NUM >= 1
static int eth_pic32cxsg_gmac_get_qav_param(const struct device *dev,
				      enum ethernet_config_type type,
				      struct ethernet_config *config)
{
	...
	case ETHERNET_QAV_PARAM_TYPE_STATUS:
		...
		return eth_pic32cxsg_gmac_get_qav_status(gmac, queue_id, enabled);
	case ETHERNET_QAV_PARAM_TYPE_IDLE_SLOPE:
		idle_slope = &config->qav_param.idle_slope;
		return eth_pic32cxsg_gmac_get_qav_idle_slope(gmac, queue_id,
						       idle_slope);
	...
	case ETHERNET_QAV_PARAM_TYPE_DELTA_BANDWIDTH:
		delta_bandwidth = &config->qav_param.delta_bandwidth;
		return eth_pic32cxsg_gmac_get_qav_delta_bandwidth(gmac, queue_id,
							    delta_bandwidth);
...
static int eth_pic32cxsg_gmac_get_config(const struct device *dev,
				   enum ethernet_config_type type,
				   struct ethernet_config *config)
{
	switch (type) {
...
#if GMAC_ACTIVE_PRIORITY_QUEUE_NUM >= 1
	case ETHERNET_CONFIG_TYPE_QAV_PARAM:
		return eth_pic32cxsg_gmac_get_qav_param(dev, type, config);
#endif
	default:
		break;
	}
...
#if defined(CONFIG_PTP_CLOCK_PIC32CXSG_GMAC)
static const struct device *eth_pic32cxsg_gmac_get_ptp_clock(const struct device *dev)
{
	struct eth_pic32cxsg_dev_data *const dev_data = dev->data;
...
static const struct ethernet_api eth_api = {
	...
	.get_capabilities = eth_pic32cxsg_gmac_get_capabilities,
	.set_config = eth_pic32cxsg_gmac_set_config,
	.get_config = eth_pic32cxsg_gmac_get_config,
	.send = eth_tx,

#if defined(CONFIG_PTP_CLOCK_PIC32CXSG_GMAC)
	.get_ptp_clock = eth_pic32cxsg_gmac_get_ptp_clock,
#endif
};
...
static const struct eth_pic32cxsg_dev_cfg eth0_config = {
	.regs = (Gmac *)DT_INST_REG_ADDR(0),
	.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(0),
#ifdef CONFIG_SOC_FAMILY_MICROCHIP_PIC32CXSG
	.clock_cfg = PIC32CXSG_DT_INST_CLOCK_PMC_CFG(0),
#endif
	.config_func = eth0_irq_config,
	.phy_dev = DEVICE_DT_GET(DT_INST_PHANDLE(0, phy_handle))
};

static struct eth_pic32cxsg_dev_data eth0_data = {
...
static int ptp_clock_pic32cxsg_gmac_set(const struct device *dev,
				  struct net_ptp_time *tm)
{
	struct ptp_context *ptp_context = dev->data;
	const struct eth_pic32cxsg_dev_cfg *const cfg = ptp_context->eth_dev->config;
	Gmac *gmac = cfg->regs;
...
static int ptp_clock_pic32cxsg_gmac_get(const struct device *dev,
				  struct net_ptp_time *tm)
{
	struct ptp_context *ptp_context = dev->data;
	const struct eth_pic32cxsg_dev_cfg *const cfg = ptp_context->eth_dev->config;
...
static int ptp_clock_pic32cxsg_gmac_adjust(const struct device *dev, int increment)
{
	struct ptp_context *ptp_context = dev->data;
	const struct eth_pic32cxsg_dev_cfg *const cfg = ptp_context->eth_dev->config;
	Gmac *gmac = cfg->regs;
...
static int ptp_clock_pic32cxsg_gmac_rate_adjust(const struct device *dev,
					  double ratio)
{
	return -ENOTSUP;
}
...
static const struct ptp_clock_driver_api ptp_api = {
	.set = ptp_clock_pic32cxsg_gmac_set,
	.get = ptp_clock_pic32cxsg_gmac_get,
	.adjust = ptp_clock_pic32cxsg_gmac_adjust,
	.rate_adjust = ptp_clock_pic32cxsg_gmac_rate_adjust,
};

static int ptp_gmac_init(const struct device *port)
{
	const struct device *const eth_dev = DEVICE_DT_INST_GET(0);
	struct eth_pic32cxsg_dev_data *dev_data = eth_dev->data;
...
#endif /* CONFIG_PTP_CLOCK_PIC32CXSG_GMAC */
