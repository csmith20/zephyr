...
#define DT_DRV_COMPAT microchip_pic32cxsg_dmac
...
LOG_MODULE_REGISTER(dma_pic32cxsg, CONFIG_DMA_LOG_LEVEL);

#define DMA_REGS	((Dmac *)DT_INST_REG_ADDR(0))

struct dma_pic32cxsg_channel {
	dma_callback_t cb;
	void *user_data;
};

struct dma_pic32cxsg_data {
	__aligned(16) DmacDescriptor descriptors[DMAC_CH_NUM];
	__aligned(16) DmacDescriptor descriptors_wb[DMAC_CH_NUM];
	struct dma_pic32cxsg_channel channels[DMAC_CH_NUM];
};

/* Handles DMA interrupts and dispatches to the individual channel */
static void dma_pic32cxsg_isr(const struct device *dev)
{
	struct dma_pic32cxsg_data *data = dev->data;
	struct dma_pic32cxsg_channel *chdata;
...
/* Configure a channel */
static int dma_pic32cxsg_config(const struct device *dev, uint32_t channel,
			   struct dma_config *config)
{
	struct dma_pic32cxsg_data *data = dev->data;
	DmacDescriptor *desc = &data->descriptors[channel];
	struct dma_block_config *block = config->head_block;
	struct dma_pic32cxsg_channel *channel_control;
	...
	/*
	 * The "bigger" DMAC on some PIC32CXSG chips (e.g. SAMD5x) has
	 ...
	 * MICROCHIP HAL (i.e. it's only defined if indirect access is required).
	 */
#ifdef DMAC_CHID_ID
...
static int dma_pic32cxsg_start(const struct device *dev, uint32_t channel)
{
...
static int dma_pic32cxsg_stop(const struct device *dev, uint32_t channel)
{
	unsigned int key = irq_lock();
...
static int dma_pic32cxsg_reload(const struct device *dev, uint32_t channel,
...
static int dma_pic32cxsg_get_status(const struct device *dev, uint32_t channel,
...
#define DMA_PIC32CXSG_IRQ_CONNECT(n)						 \
	do {								 \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(0, n, irq),		 \
			    DT_INST_IRQ_BY_IDX(0, n, priority),		 \
			    dma_pic32cxsg_isr, DEVICE_DT_INST_GET(0), 0);	 \
		irq_enable(DT_INST_IRQ_BY_IDX(0, n, irq));		 \
	} while (false)

static int dma_pic32cxsg_init(const struct device *dev)
{
	struct dma_pic32cxsg_data *data = dev->data;
	...
	DMA_PIC32CXSG_IRQ_CONNECT(0);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 1)
	DMA_PIC32CXSG_IRQ_CONNECT(1);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 2)
	DMA_PIC32CXSG_IRQ_CONNECT(2);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 3)
	DMA_PIC32CXSG_IRQ_CONNECT(3);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 4)
	DMA_PIC32CXSG_IRQ_CONNECT(4);
#endif

	return 0;
}

static struct dma_pic32cxsg_data dmac_data;

static const struct dma_driver_api dma_pic32cxsg_api = {
	.config = dma_pic32cxsg_config,
	.start = dma_pic32cxsg_start,
	.stop = dma_pic32cxsg_stop,
	.reload = dma_pic32cxsg_reload,
	.get_status = dma_pic32cxsg_get_status,
};

DEVICE_DT_INST_DEFINE(0, &dma_pic32cxsg_init, NULL,
		    &dmac_data, NULL, PRE_KERNEL_1,
		    CONFIG_DMA_INIT_PRIORITY, &dma_pic32cxsg_api);
