...
#define DT_DRV_COMPAT microchip_pic32cxsg_adc
...
LOG_MODULE_REGISTER(adc_pic32cxsg, CONFIG_ADC_LOG_LEVEL);
 ...
 */
#define ADC_PIC32CXSG_REFERENCE_GLITCH 1
#endif

struct adc_pic32cxsg_data {
...
#ifdef ADC_PIC32CXSG_REFERENCE_GLITCH
	uint8_t reference_changed;
#endif
};

struct adc_pic32cxsg_cfg {
	Adc *regs;
...
static int adc_pic32cxsg_acquisition_to_clocks(const struct device *dev,
					  uint16_t acquisition_time)
{
	const struct adc_pic32cxsg_cfg *const cfg = dev->config;
	uint64_t scaled_acq;
...
static int adc_pic32cxsg_channel_setup(const struct device *dev,
				  const struct adc_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->acquisition_time != ADC_ACQ_TIME_DEFAULT) {
		retval = adc_pic32cxsg_acquisition_to_clocks(dev,
...
#ifdef ADC_PIC32CXSG_REFERENCE_ENABLE_PROTECTED
...
#endif
#ifdef ADC_PIC32CXSG_REFERENCE_GLITCH
		struct adc_pic32cxsg_data *data = dev->data;
...
static void adc_pic32cxsg_start_conversion(const struct device *dev)
{
	const struct adc_pic32cxsg_cfg *const cfg = dev->config;
	Adc *const adc = cfg->regs;
...
{
	struct adc_pic32cxsg_data *data =
		CONTAINER_OF(ctx, struct adc_pic32cxsg_data, ctx);

	adc_pic32cxsg_start_conversion(data->dev);
}
...
static int adc_pic32cxsg_read(const struct device *dev,
			 const struct adc_sequence *sequence)
{
	struct adc_pic32cxsg_data *data = dev->data;
	int error;
...
static void adc_pic32cxsg_isr(const struct device *dev)
{
	struct adc_pic32cxsg_data *data = dev->data;
	const struct adc_pic32cxsg_cfg *const cfg = dev->config;
	Adc *const adc = cfg->regs;
	...
	if (data->reference_changed) {
		...
		adc_pic32cxsg_start_conversion(dev);
		return;
	}
#endif
...
static int adc_pic32cxsg_init(const struct device *dev)
{
	const struct adc_pic32cxsg_cfg *const cfg = dev->config;
	struct adc_pic32cxsg_data *data = dev->data;
	Adc *const adc = cfg->regs;
...
static int adc_pic32cxsg_read_async(const struct device *dev,
			       const struct adc_sequence *sequence,
...
static const struct adc_driver_api adc_pic32cxsg_api = {
	.channel_setup = adc_pic32cxsg_channel_setup,
	.read = adc_pic32cxsg_read,
#ifdef CONFIG_ADC_ASYNC
	.read_async = adc_pic32cxsg_read_async,
#endif
};
...
#define ADC_PIC32CXSG_CLOCK_CONTROL(n)					\
...
#define ADC_PIC32CXSG_CONFIGURE(n)						\
do {									\
	const struct adc_pic32cxsg_cfg *const cfg = dev->config;		\
	Adc * const adc = cfg->regs;					\
	adc->CALIB.reg = ADC_PIC32CXSG_BIASCOMP(n)				\
			 | ADC_PIC32CXSG_BIASR2R(n)				\
			 | ADC_PIC32CXSG_BIASREFBUF(n);			\
} while (false)
...
#define ADC_PIC32CXSG_DEVICE(n)						\
	PINCTRL_DT_INST_DEFINE(n);					\
	static void adc_pic32cxsg_config_##n(const struct device *dev);	\
	static const struct adc_pic32cxsg_cfg adc_sam_cfg_##n = {		\
		.regs = (Adc *)DT_INST_REG_ADDR(n),			\
		ADC_PIC32CXSG_CLOCK_CONTROL(n)				\
		.freq = UTIL_CAT(UTIL_CAT(SOC_ATMEL_PIC32CXSG_GCLK,		\
					  DT_INST_PROP(n, gclk)),	\
					  _FREQ_HZ) /			\
			DT_INST_PROP(n, prescaler),			\
		.config_func = &adc_pic32cxsg_config_##n,			\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),		\
	};								\
	static struct adc_pic32cxsg_data adc_sam_data_##n = {		\
	...
	DEVICE_DT_INST_DEFINE(n, adc_pic32cxsg_init, NULL,			\
			    &adc_sam_data_##n,				\
	...
	{								\
		...
		ADC_PIC32CXSG_CONFIGURE(n);					\
	}

DT_INST_FOREACH_STATUS_OKAY(ADC_PIC32CXSG_DEVICE)
