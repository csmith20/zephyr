...
#define DT_DRV_COMPAT atmel_pic32cxsg_i2c
...
LOG_MODULE_REGISTER(i2c_pic32cxsg, CONFIG_I2C_LOG_LEVEL);
...
struct i2c_pic32cxsg_dev_config {
	SercomI2cm *regs;
...
#ifdef CONFIG_I2C_PIC32CXSG_DMA_DRIVEN
...
struct i2c_pic32cxsg_msg {
	uint8_t *buffer;
	uint32_t size;
	uint32_t status;
};

struct i2c_pic32cxsg_dev_data {
	struct k_sem lock;
	struct k_sem sem;
	struct i2c_pic32cxsg_msg msg;
	struct i2c_msg *msgs;
	uint8_t num_msgs;
};
...
static bool i2c_pic32cxsg_terminate_on_error(const struct device *dev)
{
	struct i2c_pic32cxsg_dev_data *data = dev->data;
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
...
static void i2c_pic32cxsg_isr(const struct device *dev)
{
	struct i2c_pic32cxsg_dev_data *data = dev->data;
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	...
	if (i2c_pic32cxsg_terminate_on_error(dev)) {
		return;
	}
...
static void i2c_pic32cxsg_dma_write_done(const struct device *dma_dev, void *arg,
...
static bool i2c_pic32cxsg_dma_write_start(const struct device *dev)
{
	struct i2c_pic32cxsg_dev_data *data = dev->data;
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	...
	dma_cfg.dma_callback = i2c_pic32cxsg_dma_write_done;
...
static void i2c_pic32cxsg_dma_read_done(const struct device *dma_dev, void *arg,
				   uint32_t id, int error_code)
{
	const struct device *dev = arg;
	struct i2c_pic32cxsg_dev_data *data = dev->data;
...
static bool i2c_pic32cxsg_dma_read_start(const struct device *dev)
{
	struct i2c_pic32cxsg_dev_data *data = dev->data;
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	...
	dma_cfg.dest_data_size = 1;
	dma_cfg.user_data = (void *)dev;
	dma_cfg.dma_callback = i2c_pic32cxsg_dma_read_done;
	dma_cfg.block_count = 1;
	dma_cfg.head_block = &dma_blk;
...
static int i2c_pic32cxsg_transfer(const struct device *dev, struct i2c_msg *msgs,
...
#ifdef CONFIG_I2C_PIC32CXSG_DMA_DRIVEN
			if (!i2c_pic32cxsg_dma_read_start(dev))
#endif
			{
				i2c->INTENSET.reg = SERCOM_I2CM_INTENSET_SB;
			}
...
#ifdef CONFIG_I2C_PIC32CXSG_DMA_DRIVEN
			if (!i2c_pic32cxsg_dma_write_start(dev))
#endif
			{
				i2c->INTENSET.reg = SERCOM_I2CM_INTENSET_MB;
			}
...
static int i2c_pic32cxsg_set_apply_bitrate(const struct device *dev,
				      uint32_t config)
{
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	...
	case I2C_SPEED_STANDARD:
		...
		baud = (SOC_PIC32CXSG_GCLK0_FREQ_HZ / 100000U - 5U - 10U) / 2U;
	...
	case I2C_SPEED_FAST:
		...
		/* 5 is the nominal 100ns rise time from the app notes */
		baud = (SOC_PIC32CXSG_GCLK0_FREQ_HZ / 400000U - 5U - 10U) / 2U;
		if (baud > 255U || baud < 1U) {
			return -ERANGE;
		}
	...
	case I2C_SPEED_FAST_PLUS:
		...
		/* 5 is the nominal 100ns rise time from the app notes */
		baud = (SOC_PIC32CXSG_GCLK0_FREQ_HZ / 1000000U - 5U - 10U);
	...
	case I2C_SPEED_HIGH:
		...
		baud = (SOC_PIC32CXSG_GCLK0_FREQ_HZ / 3400000U) - 2U;
...
static int i2c_pic32cxsg_configure(const struct device *dev, uint32_t config)
{
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	int retval;
	...
	if (config & I2C_SPEED_MASK) {
		...
		retval = i2c_pic32cxsg_set_apply_bitrate(dev, config);
...
static int i2c_pic32cxsg_initialize(const struct device *dev)
{
	struct i2c_pic32cxsg_dev_data *data = dev->data;
	const struct i2c_pic32cxsg_dev_config *const cfg = dev->config;
	SercomI2cm *i2c = cfg->regs;
	...
	*cfg->mclk |= cfg->mclk_mask;
	...
	retval = i2c_pic32cxsg_set_apply_bitrate(dev,
...
static const struct i2c_driver_api i2c_pic32cxsg_driver_api = {
	.configure = i2c_pic32cxsg_configure,
	.transfer = i2c_pic32cxsg_transfer,
};

#ifdef CONFIG_I2C_PIC32CXSG_DMA_DRIVEN
#define I2C_PIC32CXSG_DMA_CHANNELS(n)					\
	.dma_dev = DEVICE_DT_GET(PIC32CXSG_DT_INST_DMA_CTLR(n, tx)),	\
	.write_dma_request = PIC32CXSG_DT_INST_DMA_TRIGSRC(n, tx),	\
	.read_dma_request = PIC32CXSG_DT_INST_DMA_TRIGSRC(n, rx),	\
	.dma_channel = PIC32CXSG_DT_INST_DMA_CHANNEL(n, rx),
#else
#define I2C_PIC32CXSG_DMA_CHANNELS(n)
#endif

#define PIC32CXSG_I2C_IRQ_CONNECT(n, m)					\
	do {								\
...
#define I2C_PIC32CXSG_IRQ_HANDLER(n)						\
static void i2c_pic32cxsg_irq_config_##n(const struct device *dev)			\
{									\
	PIC32CXSG_I2C_IRQ_CONNECT(n, 0);					\
	PIC32CXSG_I2C_IRQ_CONNECT(n, 1);					\
	PIC32CXSG_I2C_IRQ_CONNECT(n, 2);					\
	PIC32CXSG_I2C_IRQ_CONNECT(n, 3);					\
}
#else
#define I2C_PIC32CXSG_IRQ_HANDLER(n)						\
static void i2c_pic32cxsg_irq_config_##n(const struct device *dev)			\
...
#ifdef MCLK
#define I2C_PIC32CXSG_CONFIG(n)						\
static const struct i2c_pic32cxsg_dev_config i2c_pic32cxsg_dev_config_##n = {	\
	...
	.irq_config_func = &i2c_pic32cxsg_irq_config_##n,			\
	I2C_PIC32CXSG_DMA_CHANNELS(n)					\
}
#else /* !MCLK */
#define I2C_PIC32CXSG_CONFIG(n)						\
static const struct i2c_pic32cxsg_dev_config i2c_pic32cxsg_dev_config_##n = {	\
...
#define I2C_PIC32CXSG_DEVICE(n)						\
	PINCTRL_DT_INST_DEFINE(n);					\
	static void i2c_pic32cxsg_irq_config_##n(const struct device *dev);	\
	I2C_PIC32CXSG_CONFIG(n);						\
	static struct i2c_pic32cxsg_dev_data i2c_pic32cxsg_dev_data_##n;		\
	I2C_DEVICE_DT_INST_DEFINE(n,					\
			    i2c_pic32cxsg_initialize,			\
			    NULL,					\
			    &i2c_pic32cxsg_dev_data_##n,			\
			    &i2c_pic32cxsg_dev_config_##n, POST_KERNEL,	\
			    CONFIG_I2C_INIT_PRIORITY,			\
			    &i2c_pic32cxsg_driver_api);			\
	I2C_PIC32CXSG_IRQ_HANDLER(n)

DT_INST_FOREACH_STATUS_OKAY(I2C_PIC32CXSG_DEVICE)
