...
#define DT_DRV_COMPAT microchip_pic32cxsg_tc32
...
LOG_MODULE_REGISTER(counter_pic32cxsg_tc32, CONFIG_COUNTER_LOG_LEVEL);

struct counter_pic32cxsg_tc32_ch_data {
	counter_alarm_callback_t callback;
	void *user_data;
};

struct counter_pic32cxsg_tc32_data {
	counter_top_callback_t top_cb;
	void *top_user_data;

	struct counter_pic32cxsg_tc32_ch_data ch;
};

struct counter_pic32cxsg_tc32_config {
...
static int counter_pic32cxsg_tc32_start(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
...
static int counter_pic32cxsg_tc32_stop(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
...
static uint32_t counter_pic32cxsg_tc32_read(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
...
static int counter_pic32cxsg_tc32_get_value(const struct device *dev,
				       uint32_t *ticks)
{
	*ticks = counter_pic32cxsg_tc32_read(dev);
	return 0;
}

static void counter_pic32cxsg_tc32_relative_alarm(const struct device *dev,
					     uint32_t ticks)
{
	struct counter_pic32cxsg_tc32_data *data = dev->data;
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
	uint32_t before;
	uint32_t target;
...
static int counter_pic32cxsg_tc32_set_alarm(const struct device *dev,
				       uint8_t chan_id,
				       const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if ((alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) != 0) {
		...
		counter_pic32cxsg_tc32_relative_alarm(dev, alarm_cfg->ticks);
	}
...
static int counter_pic32cxsg_tc32_cancel_alarm(const struct device *dev,
					  uint8_t chan_id)
{
	struct counter_pic32cxsg_tc32_data *data = dev->data;
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
...
static int counter_pic32cxsg_tc32_set_top_value(const struct device *dev,
					   const struct counter_top_cfg *top_cfg)
{
	...
	if (top_cfg->flags & COUNTER_TOP_CFG_DONT_RESET) {
		 ...
		 */
		if (counter_pic32cxsg_tc32_read(dev) >= top_cfg->ticks) {
			err = -ETIME;
...
static uint32_t counter_pic32cxsg_tc32_get_pending_int(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
...
static uint32_t counter_pic32cxsg_tc32_get_top_value(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
...
static void counter_pic32cxsg_tc32_isr(const struct device *dev)
{
	struct counter_pic32cxsg_tc32_data *data = dev->data;
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
...
static int counter_pic32cxsg_tc32_initialize(const struct device *dev)
{
	const struct counter_pic32cxsg_tc32_config *const cfg = dev->config;
	TcCount32 *tc = cfg->regs;
	int retval;
...
static const struct counter_driver_api counter_pic32cxsg_tc32_driver_api = {
	.start = counter_pic32cxsg_tc32_start,
	.stop = counter_pic32cxsg_tc32_stop,
	.get_value = counter_pic32cxsg_tc32_get_value,
	.set_alarm = counter_pic32cxsg_tc32_set_alarm,
	.cancel_alarm = counter_pic32cxsg_tc32_cancel_alarm,
	.set_top_value = counter_pic32cxsg_tc32_set_top_value,
	.get_pending_int = counter_pic32cxsg_tc32_get_pending_int,
	.get_top_value = counter_pic32cxsg_tc32_get_top_value,
};


#ifdef MCLK
#define COUNTER_PIC32CXSG_TC32_CLOCK_CONTROL(n)				\
...
#define PIC32CXSG_TC32_PRESCALER(n)						\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, prescaler),		\
		    (DT_INST_PROP(n, prescaler)), (1))

#define COUNTER_PIC32CXSG_TC32_DEVICE(n)					\
	PINCTRL_DT_INST_DEFINE(n);					\
	static void counter_pic32cxsg_tc32_config_##n(const struct device *dev); \
	static const struct counter_pic32cxsg_tc32_config			\
									\
	counter_pic32cxsg_tc32_dev_config_##n = {				\
		.info = {						\
			.max_top_value = UINT32_MAX,			\
			.freq = SOC_MICROCHIP_PIC32CXSG_GCLK0_FREQ_HZ /		\
				PIC32CXSG_TC32_PRESCALER(n),			\
			.flags = COUNTER_CONFIG_INFO_COUNT_UP,		\
			.channels = 1					\
		},							\
		.regs = (TcCount32 *)DT_INST_REG_ADDR(n),		\
		COUNTER_PIC32CXSG_TC32_CLOCK_CONTROL(n)			\
		.prescaler = UTIL_CAT(TC_CTRLA_PRESCALER_DIV,		\
				      PIC32CXSG_TC32_PRESCALER(n)),		\
		.irq_config_func = &counter_pic32cxsg_tc32_config_##n,	\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),		\
	};								\
									\
	static struct counter_pic32cxsg_tc32_data counter_pic32cxsg_tc32_dev_data_##n;\
									\
	DEVICE_DT_INST_DEFINE(n,					\
			    &counter_pic32cxsg_tc32_initialize,		\
			    NULL,					\
			    &counter_pic32cxsg_tc32_dev_data_##n,		\
			    &counter_pic32cxsg_tc32_dev_config_##n,		\
			    PRE_KERNEL_1,				\
			    CONFIG_COUNTER_INIT_PRIORITY,		\
			    &counter_pic32cxsg_tc32_driver_api);		\
									\
	static void counter_pic32cxsg_tc32_config_##n(const struct device *dev) \
	{								\
		IRQ_CONNECT(DT_INST_IRQN(n),				\
			    DT_INST_IRQ(n, priority),			\
			    counter_pic32cxsg_tc32_isr,			\
			    DEVICE_DT_INST_GET(n), 0);			\
		irq_enable(DT_INST_IRQN(n));				\
	}

DT_INST_FOREACH_STATUS_OKAY(COUNTER_PIC32CXSG_TC32_DEVICE)
