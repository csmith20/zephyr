 ...
 */
#define DT_DRV_COMPAT microchip_pic32cxsg_spi

#define LOG_LEVEL CONFIG_SPI_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(spi_pic32cxsg);
...
static int spi_pic32cxsg_configure(const struct device *dev,
			      const struct spi_config *config)
{
	const struct spi_pic32cxsg_config *cfg = dev->config;
	struct spi_pic32cxsg_data *data = dev->data;
	...
	div = (SOC_microchip_pic32cxsg_GCLK0_FREQ_HZ / config->frequency) / 2U - 1;
...
static bool spi_pic32cxsg_transfer_ongoing(struct spi_pic32cxsg_data *data)
{
	return spi_context_tx_on(&data->ctx) || spi_context_rx_on(&data->ctx);
}

static void spi_pic32cxsg_shift_master(SercomSpi *regs, struct spi_pic32cxsg_data *data)
{
	uint8_t tx;
	uint8_t rx;
...
static void spi_pic32cxsg_finish(SercomSpi *regs)
{
...
static void spi_pic32cxsg_fast_tx(SercomSpi *regs, const struct spi_buf *tx_buf)
{
	...
	spi_pic32cxsg_finish(regs);
}

/* Fast path that reads into a buf */
static void spi_pic32cxsg_fast_rx(SercomSpi *regs, const struct spi_buf *rx_buf)
{
...
static void spi_pic32cxsg_fast_txrx(SercomSpi *regs,
...
static void spi_pic32cxsg_fast_transceive(const struct device *dev,
				     const struct spi_config *config,
				     ...
				     const struct spi_buf_set *rx_bufs)
{
	...
	while (tx_count != 0 && rx_count != 0) {
		if (tx->buf == NULL) {
			spi_pic32cxsg_fast_rx(regs, rx);
		} else if (rx->buf == NULL) {
			spi_pic32cxsg_fast_tx(regs, tx);
		} else {
			spi_pic32cxsg_fast_txrx(regs, tx, rx);
		}
 ...
 */
static bool spi_pic32cxsg_is_regular(const struct spi_buf_set *tx_bufs,
				const struct spi_buf_set *rx_bufs)
{
	const struct spi_buf *tx = NULL;
	const struct spi_buf *rx = NULL;
...
static int spi_pic32cxsg_transceive(const struct device *dev,
			       const struct spi_config *config,
			       const struct spi_buf_set *tx_bufs,
			       const struct spi_buf_set *rx_bufs)
{
	...
	err = spi_pic32cxsg_configure(dev, config);
	if (err != 0) {
		goto done;
	}
	 ...
	 */
	if (spi_pic32cxsg_is_regular(tx_bufs, rx_bufs)) {
		spi_pic32cxsg_fast_transceive(dev, config, tx_bufs, rx_bufs);
	} else {
		spi_context_buffers_setup(&data->ctx, tx_bufs, rx_bufs, 1);

		do {
			spi_pic32cxsg_shift_master(regs, data);
		} while (spi_pic32cxsg_transfer_ongoing(data));
	}
...
static int spi_pic32cxsg_transceive_sync(const struct device *dev,
				    const struct spi_config *config,
				    const struct spi_buf_set *tx_bufs,
				    const struct spi_buf_set *rx_bufs)
{
	return spi_pic32cxsg_transceive(dev, config, tx_bufs, rx_bufs);
}

#ifdef CONFIG_SPI_ASYNC

static void spi_pic32cxsg_dma_rx_done(const struct device *dma_dev, void *arg,
				 uint32_t id, int error_code);

static int spi_pic32cxsg_dma_rx_load(const struct device *dev, uint8_t *buf,
				size_t len)
{
	const struct spi_pic32cxsg_config *cfg = dev->config;
	struct spi_pic32cxsg_data *data = dev->data;
	...
	dma_cfg.dma_callback = spi_pic32cxsg_dma_rx_done;
...
static int spi_pic32cxsg_dma_tx_load(const struct device *dev, const uint8_t *buf,
				size_t len)
{
	const struct spi_pic32cxsg_config *cfg = dev->config;
	SercomSpi *regs = cfg->regs;
...
static bool spi_pic32cxsg_dma_advance_segment(const struct device *dev)
{
	struct spi_pic32cxsg_data *data = dev->data;
...
static int spi_pic32cxsg_dma_advance_buffers(const struct device *dev)
{
	struct spi_pic32cxsg_data *data = dev->data;
	int retval;
	...
	if (data->ctx.rx_len) {
		retval = spi_pic32cxsg_dma_rx_load(dev, data->ctx.rx_buf,
					      data->dma_segment_len);
	} else {
		retval = spi_pic32cxsg_dma_rx_load(dev, NULL, data->dma_segment_len);
	}
	...
	if (data->ctx.tx_len) {
		retval = spi_pic32cxsg_dma_tx_load(dev, data->ctx.tx_buf,
					      data->dma_segment_len);
	} else {
		retval = spi_pic32cxsg_dma_tx_load(dev, NULL, data->dma_segment_len);
	}
...
static void spi_pic32cxsg_dma_rx_done(const struct device *dma_dev, void *arg,
				 uint32_t id, int error_code)
{
	struct spi_pic32cxsg_data *data = arg;
	...
	if (!spi_pic32cxsg_dma_advance_segment(dev)) {
		/* Done */
		spi_context_cs_control(&data->ctx, false);
	...
	retval = spi_pic32cxsg_dma_advance_buffers(dev);
	if (retval != 0) {
...
static int spi_pic32cxsg_transceive_async(const struct device *dev,
				     const struct spi_config *config,
				     const struct spi_buf_set *tx_bufs,
				     const struct spi_buf_set *rx_bufs,
				     spi_callback_t cb,
				     void *userdata)
{
	...
	retval = spi_pic32cxsg_configure(dev, config);
	if (retval != 0) {
		goto err_unlock;
	}
	...
	spi_pic32cxsg_dma_advance_segment(dev);
	retval = spi_pic32cxsg_dma_advance_buffers(dev);
	if (retval != 0) {
		goto err_cs;
	}
...
static int spi_pic32cxsg_release(const struct device *dev,
			    const struct spi_config *config)
{
	struct spi_pic32cxsg_data *data = dev->data;
...
static int spi_pic32cxsg_init(const struct device *dev)
{
	int err;
	const struct spi_pic32cxsg_config *cfg = dev->config;
	struct spi_pic32cxsg_data *data = dev->data;
...
static const struct spi_driver_api spi_pic32cxsg_driver_api = {
	.transceive = spi_pic32cxsg_transceive_sync,
#ifdef CONFIG_SPI_ASYNC
	.transceive_async = spi_pic32cxsg_transceive_async,
#endif
	.release = spi_pic32cxsg_release,
};

#if CONFIG_SPI_ASYNC
#define SPI_pic32cxsg_DMA_CHANNELS(n)					\
...
#define SPI_pic32cxsg_SERCOM_PADS(n) \
	SERCOM_SPI_CTRLA_DIPO(DT_INST_PROP(n, dipo)) | \
	SERCOM_SPI_CTRLA_DOPO(DT_INST_PROP(n, dopo))

#ifdef MCLK
#define SPI_pic32cxsg_DEFINE_CONFIG(n)					\
static const struct spi_pic32cxsg_config spi_pic32cxsg_config_##n = {		\
	...
	.pads = SPI_pic32cxsg_SERCOM_PADS(n),				\
	.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),			\
	SPI_pic32cxsg_DMA_CHANNELS(n)					\
}
#else
#define SPI_pic32cxsg_DEFINE_CONFIG(n)					\
static const struct spi_pic32cxsg_config spi_pic32cxsg_config_##n = {		\
...
#define SPI_pic32cxsg_DEVICE_INIT(n)						\
	PINCTRL_DT_INST_DEFINE(n);					\
	SPI_pic32cxsg_DEFINE_CONFIG(n);					\
	static struct spi_pic32cxsg_data spi_pic32cxsg_dev_data_##n = {		\
		SPI_CONTEXT_INIT_LOCK(spi_pic32cxsg_dev_data_##n, ctx),	\
		SPI_CONTEXT_INIT_SYNC(spi_pic32cxsg_dev_data_##n, ctx),	\
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(n), ctx)	\
	};								\
	DEVICE_DT_INST_DEFINE(n, &spi_pic32cxsg_init, NULL,			\
			    &spi_pic32cxsg_dev_data_##n,			\
			    &spi_pic32cxsg_config_##n, POST_KERNEL,		\
			    CONFIG_SPI_INIT_PRIORITY,			\
			    &spi_pic32cxsg_driver_api);

DT_INST_FOREACH_STATUS_OKAY(SPI_pic32cxsg_DEVICE_INIT)
