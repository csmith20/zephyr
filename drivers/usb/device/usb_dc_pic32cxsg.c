...
#define DT_DRV_COMPAT microchip_pic32cxsg_usb

#define LOG_LEVEL CONFIG_USB_DRIVER_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(usb_dc_pic32cxsg);
...
#define USB_PIC32CXSG_IN_EP 0x80
...
enum usb_pic32cxsg_pcksize_size {
	USB_PIC32CXSG_PCKSIZE_SIZE_8 = 0,
	USB_PIC32CXSG_PCKSIZE_SIZE_16,
	USB_PIC32CXSG_PCKSIZE_SIZE_32,
	USB_PIC32CXSG_PCKSIZE_SIZE_64,
	USB_PIC32CXSG_PCKSIZE_SIZE_128,
	USB_PIC32CXSG_PCKSIZE_SIZE_256,
	USB_PIC32CXSG_PCKSIZE_SIZE_512,
	USB_PIC32CXSG_PCKSIZE_SIZE_1023,
};

static const uint16_t usb_pic32cxsg_pcksize_bytes[] = {
	[USB_PIC32CXSG_PCKSIZE_SIZE_8] = 8,
	[USB_PIC32CXSG_PCKSIZE_SIZE_16] = 16,
	[USB_PIC32CXSG_PCKSIZE_SIZE_32] = 32,
	[USB_PIC32CXSG_PCKSIZE_SIZE_64] = 64,
	[USB_PIC32CXSG_PCKSIZE_SIZE_128] = 128,
	[USB_PIC32CXSG_PCKSIZE_SIZE_256] = 256,
	[USB_PIC32CXSG_PCKSIZE_SIZE_512] = 512,
	[USB_PIC32CXSG_PCKSIZE_SIZE_1023] = 1023,
};

BUILD_ASSERT(ARRAY_SIZE(usb_pic32cxsg_pcksize_bytes) == 8);

struct usb_pic32cxsg_data {
...
static struct usb_pic32cxsg_data usb_pic32cxsg_data_0;
PINCTRL_DT_INST_DEFINE(0);
static const struct pinctrl_dev_config *pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(0);

static struct usb_pic32cxsg_data *usb_pic32cxsg_get_data(void)
{
	return &usb_pic32cxsg_data_0;
}

/* Handles interrupts on an endpoint */
static void usb_pic32cxsg_ep_isr(uint8_t ep)
{
	struct usb_pic32cxsg_data *data = usb_pic32cxsg_get_data();
	...
	if ((intflag & USB_DEVICE_EPINTFLAG_TRCPT1) != 0U) {
		...
		data->ep_cb[1][ep](ep | USB_PIC32CXSG_IN_EP, USB_DC_EP_DATA_IN);
...
static void usb_pic32cxsg_isr(void)
{
	struct usb_pic32cxsg_data *data = usb_pic32cxsg_get_data();
	UsbDevice *regs = &REGS->DEVICE;
...
static void usb_pic32cxsg_wait_syncbusy(void)
{
	UsbDevice *regs = &REGS->DEVICE;
...
static void usb_pic32cxsg_load_padcal(void)
{
	UsbDevice *regs = &REGS->DEVICE;
	uint32_t pad_transn;
	uint32_t pad_transp;
...
#define PIC32CXSG_USB_IRQ_CONNECT(n) 				\
	do {							\
	IRQ_CONNECT(DT_INST_IRQ_BY_IDX(0, n, irq),		\
		    DT_INST_IRQ_BY_IDX(0, n, priority),		\
		    usb_pic32cxsg_isr, 0, 0);			\
...
int usb_dc_attach(void)
{
	...
	PIC32CXSG_USB_IRQ_CONNECT(0);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 1)
	PIC32CXSG_USB_IRQ_CONNECT(1);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 2)
	PIC32CXSG_USB_IRQ_CONNECT(2);
#endif
#if DT_INST_IRQ_HAS_IDX(0, 3)
	PIC32CXSG_USB_IRQ_CONNECT(3);
#endif
...
int usb_dc_ep_configure(const struct usb_dc_ep_cfg_data *const cfg)
{
	...
	for (i = 0; i < ARRAY_SIZE(usb_pic32cxsg_pcksize_bytes); i++) {
		if (usb_pic32cxsg_pcksize_bytes[i] == cfg->ep_mps) {
			size = i;
			break;
		}
	}
...
int usb_dc_ep_write(uint8_t ep, const uint8_t *buf, uint32_t len, uint32_t *ret_bytes)
{
	...
	uint32_t capacity = usb_pic32cxsg_pcksize_bytes[
...
int usb_dc_ep_mps(const uint8_t ep)
{
	...
	if (for_in) {
		...
		return usb_pic32cxsg_pcksize_bytes[
			desc->DeviceDescBank[1].PCKSIZE.bit.SIZE];
	} else {
		/* if endpoint is not configured, this should return 0 */
