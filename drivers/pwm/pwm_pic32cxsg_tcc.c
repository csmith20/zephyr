...
/*
 * PWM driver using the PIC32CXSG Timer/Counter (TCC) in Normal PWM (NPWM) mode.
 * Supports the PIC32CXSG series.
 */

#define DT_DRV_COMPAT microchip_pic32cxsg_tcc_pwm
...
static int pwm_pic32cxsg_get_cycles_per_sec(const struct device *dev,
				       uint32_t channel, uint64_t *cycles)
{
	const struct pwm_pic32cxsg_config *const cfg = dev->config;
...
static int pwm_pic32cxsg_set_cycles(const struct device *dev, uint32_t channel,
...
static int pwm_pic32cxsg_init(const struct device *dev)
{
	const struct pwm_pic32cxsg_config *const cfg = dev->config;
	Tcc *regs = cfg->regs;
	int retval;
...
static const struct pwm_driver_api pwm_pic32cxsg_driver_api = {
	.set_cycles = pwm_pic32cxsg_set_cycles,
	.get_cycles_per_sec = pwm_pic32cxsg_get_cycles_per_sec,
};

#ifdef MCLK
#define PWM_PIC32CXSG_INIT_CLOCKS(inst)					       \
...
#define PWM_PIC32CXSG_INIT(inst)						       \
	PINCTRL_DT_INST_DEFINE(inst);					       \
	static const struct pwm_pic32cxsg_config pwm_pic32cxsg_config_##inst = {	       \
		...
		.freq = SOC_PIC32CXSG_GCLK0_FREQ_HZ /			       \
			DT_INST_PROP(inst, prescaler),			       \
		PWM_PIC32CXSG_INIT_CLOCKS(inst),				       \
	};								       \
									       \
	DEVICE_DT_INST_DEFINE(inst, &pwm_pic32cxsg_init, NULL,		       \
			    NULL, &pwm_pic32cxsg_config_##inst,		       \
			    POST_KERNEL, CONFIG_PWM_INIT_PRIORITY,	       \
			    &pwm_pic32cxsg_driver_api);

DT_INST_FOREACH_STATUS_OKAY(PWM_PIC32CXSG_INIT)
