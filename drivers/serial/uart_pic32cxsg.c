...
#define DT_DRV_COMPAT microchip_pic32cxsg_uart
...
#endif
#if CONFIG_UART_INTERRUPT_DRIVEN || CONFIG_UART_PIC32CXSG_ASYNC
	void (*irq_config_func)(const struct device *dev);
#endif
#if CONFIG_UART_PIC32CXSG_ASYNC
...
struct uart_pic32cxsg_dev_data {
	...
	const struct uart_pic32cxsg_dev_cfg *cfg;
...
static int uart_pic32cxsg_set_baudrate(SercomUsart *const usart, uint32_t baudrate,
...
static void uart_pic32cxsg_dma_tx_done(const struct device *dma_dev, void *arg,
				  uint32_t id, int error_code)
{
	...
	struct uart_pic32cxsg_dev_data *const dev_data =
		(struct uart_pic32cxsg_dev_data *const) arg;
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev_data->cfg;
...
static int uart_pic32cxsg_tx_halt(struct uart_pic32cxsg_dev_data *dev_data)
{
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev_data->cfg;
...
static void uart_pic32cxsg_tx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_pic32cxsg_dev_data *dev_data = CONTAINER_OF(dwork,
							   struct uart_pic32cxsg_dev_data, tx_timeout_work);

	uart_pic32cxsg_tx_halt(dev_data);
}

static void uart_pic32cxsg_notify_rx_processed(struct uart_pic32cxsg_dev_data *dev_data,
					  size_t processed)
{
	if (!dev_data->async_cb) {
		return;
...
static void uart_pic32cxsg_dma_rx_done(const struct device *dma_dev, void *arg,
				  uint32_t id, int error_code)
{
	ARG_UNUSED(dma_dev);
	ARG_UNUSED(id);
	...
	uart_pic32cxsg_notify_rx_processed(dev_data, dev_data->rx_len);
...
static void uart_pic32cxsg_rx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_pic32cxsg_dev_data *dev_data = CONTAINER_OF(dwork,
							   struct uart_pic32cxsg_dev_data, rx_timeout_work);
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev_data->cfg;
	SercomUsart * const regs = cfg->regs;
	struct dma_status st;
	unsigned int key = irq_lock();
	...
	if (elapsed >= dev_data->rx_timeout_time) {
		 ...
		 */
		uart_pic32cxsg_notify_rx_processed(dev_data, rx_processed);
	} else {
		/*
...
static int uart_pic32cxsg_configure(const struct device *dev,
			       const struct uart_config *new_cfg)
{
	int retval;

	const struct uart_pic32cxsg_dev_cfg *const cfg = dev->config;
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
	...
	retval = uart_pic32cxsg_set_baudrate(usart, new_cfg->baudrate,
					SOC_MICROCHIP_PIC32CXSG_GCLK0_FREQ_HZ);
	if (retval != 0) {
		return retval;
	}
...
static int uart_pic32cxsg_config_get(const struct device *dev,
...
static int uart_pic32cxsg_init(const struct device *dev)
{
	int retval;
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev->config;
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
	...
	*cfg->mclk |= cfg->mclk_mask;
	...
	retval = uart_pic32cxsg_set_baudrate(usart, cfg->baudrate,
					SOC_MICROCHIP_PIC32CXSG_GCLK0_FREQ_HZ);
	if (retval != 0) {
		return retval;
	}
...
#ifdef CONFIG_UART_PIC32CXSG_ASYNC
	dev_data->dev = dev;
	dev_data->cfg = cfg;
	...
	k_work_init_delayable(&dev_data->tx_timeout_work, uart_pic32cxsg_tx_timeout);
	k_work_init_delayable(&dev_data->rx_timeout_work, uart_pic32cxsg_rx_timeout);
	...
	if (cfg->tx_dma_channel != 0xFFU) {
		...
		dma_cfg.dma_callback = uart_pic32cxsg_dma_tx_done;
	...
	if (cfg->rx_dma_channel != 0xFFU) {
		...
		dma_cfg.dest_data_size = 1;
		dma_cfg.user_data = dev_data;
		dma_cfg.dma_callback = uart_pic32cxsg_dma_rx_done;
		dma_cfg.block_count = 1;
		dma_cfg.head_block = &dma_blk;
...
static int uart_pic32cxsg_poll_in(const struct device *dev, unsigned char *c)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
...
static void uart_pic32cxsg_poll_out(const struct device *dev, unsigned char c)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
...
static int uart_pic32cxsg_err_check(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
...
static void uart_pic32cxsg_isr(const struct device *dev)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
...
static int uart_pic32cxsg_fifo_fill(const struct device *dev,
...
static void uart_pic32cxsg_irq_tx_enable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static void uart_pic32cxsg_irq_tx_disable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_irq_tx_ready(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_irq_tx_complete(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
	SercomUsart * const regs = config->regs;
...
static void uart_pic32cxsg_irq_rx_enable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static void uart_pic32cxsg_irq_rx_disable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_irq_rx_ready(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_fifo_read(const struct device *dev, uint8_t *rx_data,
			       const int size)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_irq_is_pending(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
#if defined(SERCOM_REV500)
static void uart_pic32cxsg_irq_err_enable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static void uart_pic32cxsg_irq_err_disable(const struct device *dev)
{
	const struct uart_pic32cxsg_dev_cfg *config = dev->config;
	SercomUsart * const regs = config->regs;
...
static int uart_pic32cxsg_irq_update(const struct device *dev)
{
...
static void uart_pic32cxsg_irq_callback_set(const struct device *dev,
				       uart_irq_callback_user_data_t cb,
				       void *cb_data)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
...
#if defined(CONFIG_UART_PIC32CXSG_ASYNC) && defined(CONFIG_UART_EXCLUSIVE_API_CALLBACKS)
...
static int uart_pic32cxsg_callback_set(const struct device *dev,
...
static int uart_pic32cxsg_tx(const struct device *dev, const uint8_t *buf,
			size_t len,
			int32_t timeout)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
...
static int uart_pic32cxsg_tx_abort(const struct device *dev)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev->config;
	...
	return uart_pic32cxsg_tx_halt(dev_data);
}

static int uart_pic32cxsg_rx_enable(const struct device *dev, uint8_t *buf,
			       size_t len,
			       int32_t timeout)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
...
static int uart_pic32cxsg_rx_buf_rsp(const struct device *dev, uint8_t *buf,
				size_t len)
{
	if (len > 0xFFFFU) {
		return -EINVAL;
...
static int uart_pic32cxsg_rx_disable(const struct device *dev)
{
	struct uart_pic32cxsg_dev_data *const dev_data = dev->data;
	const struct uart_pic32cxsg_dev_cfg *const cfg = dev->config;
	SercomUsart * const regs = cfg->regs;
...
static const struct uart_driver_api uart_pic32cxsg_driver_api = {
	.poll_in = uart_pic32cxsg_poll_in,
	.poll_out = uart_pic32cxsg_poll_out,
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	.configure = uart_pic32cxsg_configure,
	.config_get = uart_pic32cxsg_config_get,
#endif
	.err_check = uart_pic32cxsg_err_check,
#if CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_pic32cxsg_fifo_fill,
	.fifo_read = uart_pic32cxsg_fifo_read,
	.irq_tx_enable = uart_pic32cxsg_irq_tx_enable,
	.irq_tx_disable = uart_pic32cxsg_irq_tx_disable,
	.irq_tx_ready = uart_pic32cxsg_irq_tx_ready,
	.irq_tx_complete = uart_pic32cxsg_irq_tx_complete,
	.irq_rx_enable = uart_pic32cxsg_irq_rx_enable,
	.irq_rx_disable = uart_pic32cxsg_irq_rx_disable,
	.irq_rx_ready = uart_pic32cxsg_irq_rx_ready,
	.irq_is_pending = uart_pic32cxsg_irq_is_pending,
#if defined(SERCOM_REV500)
	.irq_err_enable = uart_pic32cxsg_irq_err_enable,
	.irq_err_disable = uart_pic32cxsg_irq_err_disable,
#endif
	.irq_update = uart_pic32cxsg_irq_update,
	.irq_callback_set = uart_pic32cxsg_irq_callback_set,
#endif
#if CONFIG_UART_PIC32CXSG_ASYNC
	.callback_set = uart_pic32cxsg_callback_set,
	.tx = uart_pic32cxsg_tx,
	.tx_abort = uart_pic32cxsg_tx_abort,
	.rx_enable = uart_pic32cxsg_rx_enable,
	.rx_buf_rsp = uart_pic32cxsg_rx_buf_rsp,
	.rx_disable = uart_pic32cxsg_rx_disable,
#endif
};

#if CONFIG_UART_INTERRUPT_DRIVEN || CONFIG_UART_PIC32CXSG_ASYNC

#define PIC32CXSG_UART_IRQ_CONNECT(n, m)					\
	do {								\
...
#define UART_PIC32CXSG_IRQ_HANDLER_DECL(n)					\
	static void uart_pic32cxsg_irq_config_##n(const struct device *dev)
#define UART_PIC32CXSG_IRQ_HANDLER_FUNC(n)					\
	.irq_config_func = uart_pic32cxsg_irq_config_##n,

#if DT_INST_IRQ_HAS_IDX(0, 3)
#define UART_PIC32CXSG_IRQ_HANDLER(n)					\
static void uart_pic32cxsg_irq_config_##n(const struct device *dev)		\
{									\
	PIC32CXSG_UART_IRQ_CONNECT(n, 0);					\
	PIC32CXSG_UART_IRQ_CONNECT(n, 1);					\
	PIC32CXSG_UART_IRQ_CONNECT(n, 2);					\
	PIC32CXSG_UART_IRQ_CONNECT(n, 3);					\
}
#else
#define UART_PIC32CXSG_IRQ_HANDLER(n)					\
static void uart_pic32cxsg_irq_config_##n(const struct device *dev)		\
...
#if CONFIG_UART_PIC32CXSG_ASYNC
#define UART_PIC32CXSG_DMA_CHANNELS(n)					\
	.dma_dev = DEVICE_DT_GET(MICROCHIP_PIC32CXSG_DT_INST_DMA_CTLR(n, tx)),	\
	.tx_dma_request = MICROCHIP_PIC32CXSG_DT_INST_DMA_TRIGSRC(n, tx),	\
	.tx_dma_channel = MICROCHIP_PIC32CXSG_DT_INST_DMA_CHANNEL(n, tx),	\
	.rx_dma_request = MICROCHIP_PIC32CXSG_DT_INST_DMA_TRIGSRC(n, rx),	\
	.rx_dma_channel = MICROCHIP_PIC32CXSG_DT_INST_DMA_CHANNEL(n, rx),
#else
#define UART_PIC32CXSG_DMA_CHANNELS(n)
#endif

#define UART_PIC32CXSG_SERCOM_PADS(n) \
	(DT_INST_PROP(n, rxpo) << SERCOM_USART_CTRLA_RXPO_Pos) |	\
	(DT_INST_PROP(n, txpo) << SERCOM_USART_CTRLA_TXPO_Pos)

#define UART_PIC32CXSG_SERCOM_COLLISION_DETECT(n) \
	(DT_INST_PROP(n, collision_detection))

#ifdef MCLK
#define UART_PIC32CXSG_CONFIG_DEFN(n)					\
static const struct uart_pic32cxsg_dev_cfg uart_pic32cxsg_config_##n = {		\
	...
	.pads = UART_PIC32CXSG_SERCOM_PADS(n),				\
	.collision_detect = UART_PIC32CXSG_SERCOM_COLLISION_DETECT(n),	\
	.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),			\
	UART_PIC32CXSG_IRQ_HANDLER_FUNC(n)					\
	UART_PIC32CXSG_DMA_CHANNELS(n)					\
}
#else
#define UART_PIC32CXSG_CONFIG_DEFN(n)					\
static const struct uart_pic32cxsg_dev_cfg uart_pic32cxsg_config_##n = {		\
...
#define UART_PIC32CXSG_DEVICE_INIT(n)					\
PINCTRL_DT_INST_DEFINE(n);						\
static struct uart_pic32cxsg_dev_data uart_pic32cxsg_data_##n;			\
UART_PIC32CXSG_IRQ_HANDLER_DECL(n);						\
UART_PIC32CXSG_CONFIG_DEFN(n);						\
DEVICE_DT_INST_DEFINE(n, uart_pic32cxsg_init, NULL,				\
		    &uart_pic32cxsg_data_##n,				\
		    &uart_pic32cxsg_config_##n, PRE_KERNEL_1,		\
		    CONFIG_SERIAL_INIT_PRIORITY,			\
		    &uart_pic32cxsg_driver_api);				\
UART_PIC32CXSG_IRQ_HANDLER(n)

DT_INST_FOREACH_STATUS_OKAY(UART_PIC32CXSG_DEVICE_INIT)
